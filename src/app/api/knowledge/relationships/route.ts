/**
 * Knowledge Relationship API 엔드포인트
 *
 * GET  /api/knowledge/relationships - 관계 목록 조회 (페이지네이션, 필터링)
 * POST /api/knowledge/relationships - 새 관계 생성
 */

import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/lib/db/prisma';
import { Prisma } from '@/generated/prisma';
import { requireAdmin, AuthError } from '@/lib/auth/authHelpers';
import { createLogger } from '@/lib/utils/logger';
import {
  RelationshipQuerySchema,
  CreateRelationshipSchema,
} from '@/lib/validations/knowledge';

const log = createLogger('KnowledgeAPI:Relationships');

/**
 * GET /api/knowledge/relationships
 * 관계 목록 조회
 */
export async function GET(request: NextRequest) {
  try {
    await requireAdmin();

    const { searchParams } = new URL(request.url);

    const queryResult = RelationshipQuerySchema.safeParse({
      sourceComponent: searchParams.get('sourceComponent') || undefined,
      targetComponent: searchParams.get('targetComponent') || undefined,
      relationshipType: searchParams.get('relationshipType') || undefined,
      strength: searchParams.get('strength') || undefined,
      search: searchParams.get('search') || undefined,
      isActive: searchParams.get('isActive') || undefined,
      tags: searchParams.get('tags') || undefined,
      page: searchParams.get('page') || 1,
      limit: searchParams.get('limit') || 20,
      sortBy: searchParams.get('sortBy') || 'createdAt',
      sortOrder: searchParams.get('sortOrder') || 'desc',
    });

    if (!queryResult.success) {
      return NextResponse.json(
        { error: '잘못된 쿼리 파라미터', details: queryResult.error.flatten() },
        { status: 400 }
      );
    }

    const {
      sourceComponent,
      targetComponent,
      relationshipType,
      strength,
      search,
      isActive,
      page,
      limit,
      sortBy,
      sortOrder,
    } = queryResult.data;

    const where: Prisma.KnowledgeRelationshipWhereInput = {};

    if (isActive !== undefined) {
      where.isActive = isActive;
    } else {
      where.isActive = true;
    }

    if (sourceComponent) {
      where.sourceComponent = sourceComponent;
    }

    if (targetComponent) {
      where.targetComponent = targetComponent;
    }

    if (relationshipType) {
      where.relationshipType = relationshipType;
    }

    if (strength) {
      where.strength = strength;
    }

    if (search) {
      where.OR = [
        { knowledgeId: { contains: search, mode: 'insensitive' } },
        { sourceComponent: { contains: search, mode: 'insensitive' } },
        { targetComponent: { contains: search, mode: 'insensitive' } },
        { reason: { contains: search, mode: 'insensitive' } },
        { reasonKo: { contains: search, mode: 'insensitive' } },
      ];
    }

    const total = await prisma.knowledgeRelationship.count({ where });

    const data = await prisma.knowledgeRelationship.findMany({
      where,
      orderBy: { [sortBy]: sortOrder },
      skip: (page - 1) * limit,
      take: limit,
    });

    const totalPages = Math.ceil(total / limit);

    return NextResponse.json({
      data,
      pagination: {
        page,
        limit,
        total,
        totalPages,
        hasNext: page < totalPages,
        hasPrev: page > 1,
      },
    });
  } catch (error) {
    if (error instanceof AuthError) {
      return NextResponse.json({ error: error.message }, { status: error.statusCode });
    }
    log.error('관계 목록 조회 실패', error instanceof Error ? error : undefined);
    return NextResponse.json(
      { error: '관계 목록 조회에 실패했습니다' },
      { status: 500 }
    );
  }
}

/**
 * POST /api/knowledge/relationships
 * 새 관계 생성
 */
export async function POST(request: NextRequest) {
  try {
    await requireAdmin();

    const body = await request.json();

    const validationResult = CreateRelationshipSchema.safeParse(body);

    if (!validationResult.success) {
      return NextResponse.json(
        { error: '유효하지 않은 입력 데이터', details: validationResult.error.flatten() },
        { status: 400 }
      );
    }

    const data = validationResult.data;

    // knowledgeId 중복 확인
    const existing = await prisma.knowledgeRelationship.findUnique({
      where: { knowledgeId: data.knowledgeId },
    });

    if (existing) {
      return NextResponse.json(
        { error: `knowledgeId '${data.knowledgeId}'가 이미 존재합니다` },
        { status: 409 }
      );
    }

    const relationship = await prisma.knowledgeRelationship.create({
      data: {
        knowledgeId: data.knowledgeId,
        sourceComponent: data.sourceComponent,
        targetComponent: data.targetComponent,
        relationshipType: data.relationshipType,
        strength: data.strength,
        direction: data.direction,
        reason: data.reason,
        reasonKo: data.reasonKo,
        tags: data.tags,
        trustMetadata: data.trustMetadata as Prisma.InputJsonValue,
        isActive: true,
      },
    });

    return NextResponse.json(relationship, { status: 201 });
  } catch (error) {
    if (error instanceof AuthError) {
      return NextResponse.json({ error: error.message }, { status: error.statusCode });
    }
    log.error('관계 생성 실패', error instanceof Error ? error : undefined);
    return NextResponse.json(
      { error: '관계 생성에 실패했습니다' },
      { status: 500 }
    );
  }
}
