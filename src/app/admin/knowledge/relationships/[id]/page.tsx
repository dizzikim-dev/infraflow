  ...
  if (error || !data) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        ...
        <Link href="/admin/knowledge/relationships" className="mt-4 inline-block text-blue-600 hover:underline">
          목록으로 돌아가기
        </Link>
      </div>
    );
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            ...
            <span>/</span>
            <span>{data.knowledgeId}</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">{data.knowledgeId}</h1>
          <p className="mt-1 text-sm text-gray-500">
            {data.sourceComponent} → {data.targetComponent}
          </p>
        </div>
        <div className="flex gap-3">
          ...
          <Link
            href={`/admin/knowledge/relationships/${data.id}/edit`}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <dt className="text-sm font-medium text-gray-500">Knowledge ID</dt>
                <dd className="mt-1 text-sm text-gray-900 font-mono">{data.knowledgeId}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">소스 컴포넌트</dt>
                <dd className="mt-1 text-sm text-gray-900">{data.sourceComponent}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">타겟 컴포넌트</dt>
                <dd className="mt-1 text-sm text-gray-900">{data.targetComponent}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">관계 유형</dt>
                <dd className="mt-1">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${relationshipTypeLabels[data.relationshipType]?.color || 'bg-gray-100 text-gray-800'}`}>
                    {relationshipTypeLabels[data.relationshipType]?.label || data.relationshipType}
                  </span>
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">강도</dt>
                <dd className="mt-1">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${strengthLabels[data.strength]?.color || 'bg-gray-100 text-gray-800'}`}>
                    {strengthLabels[data.strength]?.label || data.strength}
                  </span>
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">방향</dt>
                <dd className="mt-1 text-sm text-gray-900">{data.direction}</dd>
              </div>
            </dl>
          </div>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-900">{data.reasonKo || '-'}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500 mb-1">English</h3>
                <p className="text-sm text-gray-900">{data.reason || '-'}</p>
              </div>
            </div>
          </div>
