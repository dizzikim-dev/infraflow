...
## 13. 조건부 재검토 트리거 (Re-evaluation Triggers)

> GPT 피드백 Round 2 (2026-02-21)에서 "안 한다가 아니라 지금은 안 한다(조건부)"로 표현하라는 제안을 수용하여 추가.

현재 "수용 불필요" 또는 "장기 검토"로 판정한 항목들의 **재검토 조건**:

### 그래프 DB (Neo4j / ArangoDB)

| 조건 | 설명 |
|------|------|
| 3+ 홉 연쇄 영향 분석이 핵심 기능이 될 때 | "이 방화벽 룰 변경이 최종 서비스에 미치는 영향"을 연쇄적으로 보여줘야 하는 경우 |
| 엔티티 수 5,000+ 이상으로 증가 | 현재 ~800개. 탐색 쿼리가 성능 병목이 되는 시점 |
| 양방향 그래프 탐색이 반복 패턴이 될 때 | 현재는 1-2홉 직접 매칭으로 충분 |

### RAG (벡터 검색)

| 조건 | 설명 |
|------|------|
| 지식 엔트리 1,000개 이상 | 현재 ~800개, 점근 중 |
| 자유형 질문 수요 발생 | "VXLAN 관련 RFC?", "Cisco ASA vs PA-5200 비교" 등 |
| 영속 DB 존재 | Postgres 또는 SQLite 도입 시점 |

### 표준 온톨로지 (RDF/OWL)

| 조건 | 설명 |
|------|------|
| 외부 시스템 연동 요구 | NetBox/CMDB/티켓/보안 거버넌스 시스템과 양방향 동기화 |
| 표준 교환 포맷 요구 | 파트너/벤더/고객사가 "표준 형태로 내보내서 연동" 요청 |

### Policy as Code (OPA)

| 조건 | 설명 |
|------|------|
| 정책 규칙 100개 이상 | 현재 anti-pattern 45개 + compliance preset 5개 수준 |
| 핫 리로드 배포 없이 규칙 변경 필요 | 현재는 코드 배포로 규칙 반영 |

### 데이터 드리프트 관리 (`nextReviewDue`)

| 조건 | 설명 |
|------|------|
| 자동화된 갱신 워크플로우 구축 시 | 필드만 있고 워크플로우 없으면 죽은 데이터 |
| 프로덕션 배포 후 실사용자 존재 시 | 개발 중에는 `lastReviewedAt`로 충분 |

---

## 14. GPT 피드백 Round 2 반영 결과 (2026-02-21)

| GPT 제안 | 반영 방식 | 결과 |
|---|---|---|
| 벤더 다양성 룰 | 알고리즘 리랭킹 → **프레젠테이션 방식으로 변경** | EvidencePanel에 "벤더별 비교" 토글 뷰 추가 |
| 조건부 재검토 트리거 문서화 | 수용 | 본 문서 §13에 추가 |
| `nextReviewDue` 필드 | 보류 | 갱신 워크플로우 없이 필드만 추가하면 무의미 |
| Evals golden test set | 보류 | LLM 프로덕션화 이후 검토 |
